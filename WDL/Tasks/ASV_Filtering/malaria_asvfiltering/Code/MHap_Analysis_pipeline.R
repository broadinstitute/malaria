...
# 1.Parameters to export data and results -----
...
# 1.Parameters to remove or mask ASVs in the genotype table -----
...
# 1.Parameters to remove samples or loci with low performance ----
...
# 1.Parameters to filter populations or samples -----
...
# 1.Parameters to define reports that will be printed ----
...
# 1.Parameters for IBD and Connectivity report -----
...
# 2.Check packages and functions----
...
# 2.Define random color palette for all plots with multiple categorical variables----
qual_col_pals = brewer.pal.info[brewer.pal.info$category == 'qual',]
col_vector = unlist(mapply(brewer.pal, qual_col_pals$maxcolors, rownames(qual_col_pals)))

# 3.UPLOAD CIGAR OBJECT AND CONVERT TO AMPSEQ OBJECT ----

print("starting to upload genotypic data")

# Upload microhaplotype data and attaches metadata. This section of the code
# follows a if / else if / else if ... structure defined by the format of the
# cigar tables (tsv, json, excel, etc). Each block within each if statement
# follows the same structure. Load cigar information -> Load performance 
# information.
if(!is.null(cigar_paths)|!is.null(cigar_files)){ 
  ...
  print(rownames(cigar_object@cigar_table))
  print(colnames(cigar_object@cigar_table))
  
  print(rownames(ampseq_object@gt))
  print(colnames(ampseq_object@gt))
  
}else if(!is.null(ampseq_jsonfile)){# In development 
...
# 4. CLEANING, FILTERING AND ADDING METADATA TO THE AMPSEQ_OBJECT ----
...
if(PerformanceReport){
  ...
  if(!is.null(metadata_file)){
    ...
    if(sum(duplicated(external_metadata$Sample_id)) > 0){
      print(paste0('There are ', sum(duplicated(external_metadata$Sample_id)), 
                   ' duplicated Ids that are going to be removed'))
  ...
  # 4.Sample performance by different coverage----
  ...
  # 4.Overall sample performance by different coverage per Run----
  ...
  # 4.Overall sample performance by different coverage per Variable1----
...
# Note: From 5 to X all if statement follow the same pattern
# 5.Identification of off target-products and PCR artifacts----
# Off-target products----
# Notice: There is a if statement after the else for when the Performance report
# is requested.
if(!is.null(off_target_formula)){
  ...
  # parse off_target_formula_check
  # Subsequent if statements for flanking_INDEL, dINDELs_ij, nINDELs_ij, dSNVs_ij
  # dVSITES_ij, nVSITES_ij, h_ij, h_ijminor, p_ij, P_ij, H_ij, H_ijminor
...
# 6. Identification of products with INDELs in their flanking region----
if(!is.null(flanking_INDEL_formula)){
  
  flanking_INDEL_stats = get_ASVs_attributes(obj = ampseq_object, ref_fasta = amplicon_fasta, homopolymer_length = 5)
  ...
  if(n_flanking_INDEL_alleles > 0){
    ...
    ampseq_object2 = mask_alt_alleles(obj = ampseq_object, mask_formula = flanking_INDEL_formula, ref_fasta = amplicon_fasta, homopolymer_length = 5)
    
    #consistency_between_gt_and_asvtab(ampseq_object2)
...
# 7. Systematic PCR Errors SNVs in homopolymers ----
if(!is.null(SNV_in_homopolymer_formula)){
...
# 8. Systematic PCR Errors INDELs in homopolymers ----
if(!is.null(INDEL_in_homopolymer_formula)){
...
# 9. Systematic PCR Errors bimeras ----
if(!is.null(bimera_formula)){
...
# 10. Stochastic PCR Errors present as heterozygous----
...
# 11. Coverage by sample and amplicon----

if(!is.null(metadata_file)){
  # 12. Merge the external metadata with  ampseq_object ----
...
# 12. Printing performance report----
if(PerformanceReport == TRUE){
...
print(rownames(ampseq_object@gt))
print(colnames(ampseq_object@gt))
print(colnames(ampseq_object@vcf_like))
...
# 14. REPORTS IN DEVELOPMENT. LIKELY TO BE DEPRECATED
