...
def trim_primer(sampleid, fileF, fileR, res_dir, subdir, forward_primers_file, reverse_primers_file, prefix, keep_untrimmed=False):
	"""
	...
	subdir (str): The name of the subdirectory within the results directory where output files should be written
	forward_primers_file (str): Path to primer sequence file for forward read.
	reverse_primers_file (str): Path to primer sequence file for reverse read.
	...
	if os.path.isfile(fileF) and os.path.isfile(fileR):
		...
		cmd = ['cutadapt', '-g', f'file:{forward_primers_file}', '-G', f'file:{reverse_primers_file}',
...
def demultiplex_per_size(sampleid, fileF, fileR, forward_primers_file, reverse_primers_file, res_dir, subdir, read_size_fw, read_size_rv, asv_lengths, ci, sample_dict, mismatches = 2):
	"""
	...
	forward_primers_file (str): Path to the forward primer FASTA file.
	reverse_primers_file (str): Path to the reverse primer FASTA file.
	...
	if ci:
		with open(forward_primers_file, 'r') as forward_fasta:
			for record in SeqIO.parse(forward_fasta, 'fasta'):
				fw_asvs.append(record.id)
		targets_fw = len(set(fw_asvs))
    
		with open(reverse_primers_file, 'r') as reverse_fasta:
