...
code_environment = 'local' #TESTING PARAMETERS
wd = '/Users/jar4142/Desktop/WDL_AMPSEQ_TESTER_REPO/Report_Layouting' #TESTING PARAMETERS
fd = '/Users/jar4142/Desktop/malaria/WDL/Tasks/PCA/malaria_PCA/Code/'  #TESTING PARAMETERS
rd = '/Users/jar4142/Desktop/WDL_AMPSEQ_TESTER_REPO/Report_Layouting/'
output = "MHap_Profile"
ampseq_export_format = NULL
sample_ampl_rate = 0.2
locus_ampl_rate = 0.2
selected_checkboxes = "/Users/jar4142/Desktop/WDL_AMPSEQ_TESTER_REPO/Report_Layouting/selected_checkboxes_mozambique.csv"
join_by <- NULL  #TESTING PARAMETERS
Variable1 <- "Country"  #TESTING PARAMETER
Variable2 <- "Run"  #TESTING PARAMETER
Longitude <- "Longitude"  #TESTING PARAMETER
Latitude <- "Latitude" #TESTING PARAMETER
PerformanceReport <- FALSE
Drug_Surveillance_Report <- FALSE
Variants_of_Interest_Report <- FALSE
ibd_thres <- 0.9 #Double Betwen 0 and 1.
...
reference_alleles = "drug_alleles_who.csv"
hap_color_palette <- "random"  #TESTING PARAMETER
drugs <- c("Artemisinin", "Chloroquine", "Pyrimethamine", "Sulfadoxine", "Lumefantrine", "Mefloquine") #TESTING PARAMETER
include_all_drug_markers <- TRUE  #TESTING PARAMETER
na_var_rm <- TRUE  #TESTING PARAMETER
na_hap_rm <- TRUE  #TESTING PARAMETER
parallel <- FALSE  #TESTING PARAMETER
nchunks <- 100  #TESTING PARAMETER
ibd_ncol <- 4  #TESTING PARAMETER
pop_levels <- NULL #TESTING PARAMETER
poly_quantile <- NULL #TESTING PARAMETER
amplicon_fasta <- NULL
pairwise_relatedness_table <- NULL
ampseq_excelfile = 'Colombia_Data_Development_minimal/'
...
if(FALSE) {ref_fasta = args$ref_fasta}
...
if(FALSE){amplicon_fasta = args$amplicon_fasta
...
if(FALSE){reference_alleles = args$reference_alleles}
...
if(FALSE){include_all_drug_markers = as.logical(args$include_all_drug_markers)
...
if(FALSE) {pop_levels = args$pop_levels
...
if (dir.exists(ampseq_excelfile)) {
  ...
  excel_files <- list.files(ampseq_excelfile, pattern = "\\.xlsx$", full.names = TRUE)
  ...
  if (length(excel_files) > 1) {
    ...
    print("Loaded a single ampseq object from the Excel file.")
  } else {
  ...
  print("Loaded ampseq object from the specified Excel file.")
}
...
#REMOVE THIS LATER
#ampseq_object@metadata$Country[1:15] = "Kenya"
#ampseq_object@metadata$Run[1:15] = "Run0"
