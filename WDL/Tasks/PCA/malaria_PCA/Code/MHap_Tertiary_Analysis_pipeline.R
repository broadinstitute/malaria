...
parser$add_argument("-preserve_samples", "--preserve_samples", default = 'null',
                   help="Samples to preserve in the analysis.")
...
samples_to_keep = "/Users/jar4142/Desktop/WDL_AMPSEQ_TESTER_REPO/PCA/Colombia_Data_Development_minimal/preserve_samples.csv"
...
parallel <- TRUE  #TESTING PARAMETER
...
#Obtain sample list to keep
samples_to_keep = if(args$preserve_samples == 'null'){NULL}else{args$preserve_samples}
...
metadata_file = if(metadata_file == 'null'){NULL}else{
    if(tolower(code_environment) == 'local'){
...
pairwise_relatedness_table = if(pairwise_relatedness_table == 'null'){NULL}else{
    if(tolower(code_environment) == 'local'){
...
poly_quantile = if(poly_quantile == 'null'){NULL}else{
